...
select st_union(veje.geom) as geom from(
...
create table intervals_calc as
select
iid, route_id, dx, x_start, x_end, t_start, t_end, horizontal_accuracy,
...
create table clean_routes as 
select rc.*, st_distance(st_transform(rc.geom, 3044), st_transform(i.geom, 3044)) as infra_dist from route_calc rc, infrastruktur i 
where rc.v < 45 and st_distance(st_transform(rc.geom, 3044), st_transform(i.geom, 3044)) > 50;

--- Laver ruter vi som udgangspunkt ikke er interesseret i (hastighed mere end 45 km/t og indenfor 50 meter fra infrastruktur)
drop table if exists dirty_routes;

create table dirty_routes as 
select rc.*, st_distance(st_transform(rc.geom, 3044), st_transform(i.geom, 3044)) as infra_dist from route_calc rc, infrastruktur i 
where rc.v > 45 and rc.v > 0 or st_distance(st_transform(rc.geom, 3044), st_transform(i.geom, 3044)) < 50;


create table stationary_routes as 
select rc.* from route_calc rc
where rc.v = 0;

create table missing_routes as 
select * from route r 
where r.route_id in (
with temp_table as (
...
select route_id from temp_table
)
